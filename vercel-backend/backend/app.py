"""
Main Flask application for the ThinkPrint backend (serverless version).

This module is identical to ``thinkprint/backend/app.py`` in the root of
the repository.  It is copied into the ``vercel-backend`` package so
that Vercel can import it directly when running the serverless
deployment.  See the original file for detailed documentation.
"""
